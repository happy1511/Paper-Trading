...
import { topgainersandloosers } from "../modules/FetchingAxios";
...
const TrendingMarkets = () => {
  ...
  const TrendingM = async () => {
    try {
      const response = await topgainersandloosers("Nifty 50");
  ...
  return (
    <>
      <div className="Gainers">
        <ul className="Gul">
          <li>
            <h4>Nifty 50 Gainers</h4>
          </li>
          {Gainers.length !== 0 ? (
            Gainers.slice(0, 10).map((data) => (
              <li key={data.symbol}>
                {" "}
                {/* Added a key prop to the list items */}
                <div className="logoS">
                  <img
                    src={`https://unpkg.com/@extra-isin/logos@1.0.0/data/${data?.meta?.isin}.png`}
                    alt=""
                    onError={(e) => {
            ...
            <>
              <li style={{ display: "block" }}>
                <h1 style={{ textAlign: "center" }}>No Gainers</h1>
              </li>
            </>
          )}
        </ul>
      ...
      <div className="Loosers">
        <ul className="Gul">
          ...
          {Loosers.length !== 0 ? (
            ...
            <>
              <li style={{ display: "block" }}>
                <h1 style={{ textAlign: "center" }}>No Loosers</h1>
              </li>
            </>
          )}
        </ul>
      ...
      <a href="/TrendingMarkets" className="ButtonTrendingMarketLink">
        <div className="ButtonTrendingMarket">Trending Markets</div>
      </a>
    </>
  );
};
